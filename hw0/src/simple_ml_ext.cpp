...
namespace py = pybind11;
 ...
 template<typename T1, typename T2, typename T3>
    ...
    void transpose(const T1* x, T1* target, int m, int n) {
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                target[j * m + i] = x[i * n + j];
           }
        }
    }
template<typename T>
void printMatrix(const T* x, int m, int n) {
    for (int i = 0; i < m; i++) {
        float sum = 0;
        for (int j = 0; j < n; j++) {
            sum += x[i * n + j];
								  ...
								  float lr, size_t batch)
{
    ...
    int *I_y = new int[m * k];
    memset(I_y, 0, sizeof(int) * k * m);
    float *delta = new float[n * k];
    for (size_t i = 0; i < m; i++) {
        I_y[i * k + y[i]] = 1;
    }
    for (size_t b_index = 0; b_index * batch < m; b_index++) {
        int b_num = m - b_index * batch >= batch ? batch : m - b_index * batch;
        const float *x_batch = &(X[(b_index - 1) * b_num * n + b_num * n]);
        int *I_y_batch = &(I_y[(b_index - 1) * b_num * k + b_num * k]);
        matmul(x_batch, theta, x_theta, b_num, n, k);
        for (int i = 0; i < b_num; i++) {
            for (size_t j = 0; j < k; j++) {
                x_theta[i * k + j] = std::exp(x_theta[i * k + j]);
            }
        }
        float sum = 0;
        for (int i = 0; i < b_num; i++) {
