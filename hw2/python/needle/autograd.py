...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        v_node = array_api.sum(node_to_output_grads_list[node])
        node.grad = v_node
        if node.op is not None:
            v_ij_tuple = node.op.gradient_as_tuple(v_node, node)
            for pre_node, v_ij in zip(node.inputs, v_ij_tuple):
                if node_to_output_grads_list.get(pre_node):
                    node_to_output_grads_list[pre_node].append(v_ij)
                else:
                    node_to_output_grads_list[pre_node] = [v_ij]
    ### END YOUR SOLUTION
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for node in node_list:
        topo_sort_dfs(node, visited, ans)
    return ans
    ### END YOUR SOLUTION
